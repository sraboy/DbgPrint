...
Copyleft (L) 2012  AlterWare Beast Inc.   >;->

Module Name:        vsnprintf.cpp
Abstract:           
Authors:            Alter
Environment:        any
...
#define FILE_AS_STRING

#ifndef INT_MAX
  #define UINT_MAX ((unsigned int)((int)(-1)))
//  #define INT_MAX ((int)((UINT_MAX ^ (UINT_MAX >> 1)) - 1))
  #define INT_MAX ((int)(UINT_MAX >> 1))
#endif 

#ifndef MB_LEN_MAX
  #define MB_LEN_MAX 512
#endif 

#ifdef FILE_AS_STRING
#define FILE   STRING
#endif //FILE_AS_STRING

/***
*output.c - printf style output to a FILE
*
* derived from VC 6.0 CRT
***/
...
#define find_char_class(c)  ((CHARTYPE)(    \
        ((c) < _T(' ') || (c) > _T('x') ? \
            CH_OTHER            \
            :               \
        __lookuptable[(c)-_T(' ')] & 0xF)  ))

#define find_next_state(class, state)  ((STATE)(  \
    ...
    )
{
    ...
    CHARTYPE chclass; /* class of current character */
        ...
        case ST_TYPE:
            ...
            case _T('c'): {
                ...
                if (flags & (FL_LONG|FL_WIDECHAR)) {
                    ...
                    // We are going to be IRQL-safe

                    //textlen = wctomb(buffer.sz, wchar);
                    if(wchar && !(wchar && 0xff)) {
                        wchar = '?';
                    }
                    buffer.sz[0] = (char)wchar;
                    textlen = 1;
...
//                        no_output = 1;
            ...
            case _T('Z'): {
                ...
                pstr = (PSTRING)get_ptr_arg(&argptr);
            ...
            case _T('s'): {
                ...
                text.sz = (PCHAR)get_ptr_arg(&argptr);
...
{
#ifndef FILE_AS_STRING
    if (_putc_lk(ch, f) == EOF)
        *pnumwritten = -1;
    else
        ++(*pnumwritten);
#else // FILE_AS_STRING
    if(f->Length < f->MaximumLength) {
        f->Buffer[f->Length] = (char)ch;
        f->Length++;
        ++(*pnumwritten);
    } else {
        *pnumwritten = -1;
