...
BOOLEAN DbgKdpUse64bit = FALSE;

ULONG DbgKdpNumberProcessors = 1;

ptrDbgKdpPrint          DbgKdpPrint = NULL;
ptrDbgKdpGetConsoleByte DbgKdpGetConsoleByte = NULL;
...
//    USHORT Length;
    ...
    if(!DbgKdpInitComPort(ComPort, Baud)) {
        return -1;
    }
    DbgKdpPacketExpected = INITIAL_PACKET_ID;
    DbgKdpNextPacketToSend = INITIAL_PACKET_ID;
/*
    // no keyboard support in this lib !
    if(!DbgKdpStartThreads()) {
        return -1;
    }
*/
    return ERROR_SUCCESS;
} // end DbgKdConnectAndInitialize()

VOID
...

{
        ...
        switch ( (USHORT) StateChange->NewState ) {
            ...
            case DbgKdLoadSymbolsStateChange:
                if ( BufferLength < LocalStateChange->u.LoadSymbols.PathNameLength ) {
                    ...
                    //printf("  LoadSymbols.PathNameLength %x\n", 
                    //    LocalStateChange->u.LoadSymbols.PathNameLength);
    ...
    if(st == ERROR_SUCCESS) {
        DbgKdpNumberProcessors = StateChange->NumberProcessors;
    }
    return st;
} // end DbgKdWaitStateChange()

DWORD
NTAPI
DbgKdContinue (
...

{
    DBGKD_MANIPULATE_STATE m;
    PDBGKD_MANIPULATE_STATE Reply;
    PDBGKD_GET_VERSION v = &m.u.GetVersion;
    DWORD st;
    BOOLEAN rc;

    memset(v, 0, sizeof(DBGKD_GET_VERSION));
    ...
    // If this is not a GetVersion response than protocol is screwed up.
    // assert that protocol is ok.

    st = Reply->ReturnStatus;
    assert(Reply->ApiNumber == DbgKdGetVersionApi);

    // Reset message address to reply.

    v = &Reply->u.GetVersion;
    ...
    memcpy(VersionBuffer, v, sizeof(*Reply));
/*
    printf("OS Version %d.%d (proto %x, flags %x)\n", v->MajorVersion, v->MinorVersion, v->ProtocolVersion, v->Flags);
    printf("KernBase %x\n", v->KernBase);
    printf("PsLoadedModuleList %x\n", v->PsLoadedModuleList);
    printf("MachineType %x\n", v->MachineType);
*/
    return st;
}
