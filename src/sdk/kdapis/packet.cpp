...
UCHAR DbgKdpPacket32[PACKET_MAX_SIZE];
    ...
    )
{
    ...
    if(DbgKdpUse64bit && (PacketType == PACKET_TYPE_KD_STATE_MANIPULATE)) {
        memcpy(DbgKdpPacket32, PacketData, PacketDataLength);
        PacketDataLength = (USHORT)DbgkdManipulateState32To64((PDBGKD_MANIPULATE_STATE)PacketData, (PDBGKD_MANIPULATE_STATE64)&DbgKdpPacket32, &AdditionalDataSize);
/*
        memcpy(&(DbgKdpPacket32[sizeof(DBGKD_MANIPULATE_STATE64))],
               (PUCHAR)PacketData+sizeof(DBGKD_MANIPULATE_STATE),
               PacketDataLength - sizeof(DBGKD_MANIPULATE_STATE));
*/
        PacketData = (PDBGKD_MANIPULATE_STATE)&DbgKdpPacket32;
    }
...
WaitForPacketLeader:
    ...
    if (PacketHeader.PacketType == PACKET_TYPE_KD_POLL_BREAKIN) {
        printf("break-in poll\n");
        KdDump(DbgKdpPacket, PacketHeader.ByteCount);
        goto AskForResend;
    } else
    if (PacketHeader.PacketType == PACKET_TYPE_KD_TRACE_IO) {
        printf("trace i/o (not handled)\n");
        KdDump(DbgKdpPacket, PacketHeader.ByteCount);
        goto AskForResend;
    } else
    if (PacketHeader.PacketType == PACKET_TYPE_KD_CONTROL_REQUEST) {
        printf("control request (not handled)\n");
        KdDump(DbgKdpPacket, PacketHeader.ByteCount);
        goto AskForResend;
    } else
    if (PacketHeader.PacketType == PACKET_TYPE_KD_FILE_IO) {
        printf("file i/o\n");
        //KdDump(DbgKdpPacket, PacketHeader.ByteCount);
        IoFile = (PDBGKD_FILE_IO)DbgKdpPacket;

/*
        if (IoFile->ApiNumber == DbgKdCreateFileApi) {
            DbgKdpOpenFile(
                  IoFile->,
                  );
        }
*/      
        switch(IoFile->ApiNumber) {
        case DbgKdCreateFileApi:
            printf("DbgKdCreateFileApi\n");
            printf("%S\n", IoFile+1);
            printf("DesiredAccess %#x, FileAttributes %#x, ShareAccess %#x, CreateDisposition %#x\n",
                IoFile->u.CreateFile.DesiredAccess,
                IoFile->u.CreateFile.FileAttributes,
                IoFile->u.CreateFile.ShareAccess,
                IoFile->u.CreateFile.CreateDisposition
                );
            break;
        case DbgKdReadFileApi:
            printf("DbgKdReadFileApi\n"); break;
        case DbgKdWriteFileApi:
            printf("DbgKdWriteFileApi\n"); break;
        case DbgKdCloseFileApi:
            printf("DbgKdCloseFileApi\n"); break;
        }

#define STATUS_ACCESS_DENIED             (0xC0000022L)

        memset(&(IoFile->u), 0, sizeof(IoFile->u));
        IoFile->Status = STATUS_ACCESS_DENIED /*CP_GET_ERROR*/;
        switch(IoFile->ApiNumber) {
        case DbgKdCreateFileApi:
            IoFile->u.CreateFile.Handle = -1;
            break;
        }
        DbgKdpWritePacket(IoFile, sizeof(DBGKD_FILE_IO),
...
ReadBuffered:
    ...
    //
    // Check if PacketType is what we are waiting for.
    //

    //KdDump(DbgKdpPacket, PacketHeader.ByteCount);
    if (PacketType != PacketHeader.PacketType) {
        if(PacketType == PACKET_TYPE_KD_STATE_CHANGE &&
           PacketHeader.PacketType != PACKET_TYPE_KD_STATE_CHANGE64) {
            goto WaitForPacketLeader;
        }
        if(!DbgKdpUse64bit) {
            //printf("64-bit state\n");
        ...
        //printf("1 ByteCount %x, sz(32) %x, sz(64) %x, ctx(32) %x, ctx(64) %x\n", PacketHeader.ByteCount, 
        //    sizeof(DBGKD_WAIT_STATE_CHANGE), sizeof(DBGKD_WAIT_STATE_CHANGE64),
        //    offsetof(DBGKD_WAIT_STATE_CHANGE, Context),
        //    offsetof(DBGKD_WAIT_STATE_CHANGE64, Context)
        //    );
        //KdDump(DbgKdpPacket, PacketHeader.ByteCount);
        WaitStateChange64To32((PDBGKD_WAIT_STATE_CHANGE64)DbgKdpPacket, (PDBGKD_WAIT_STATE_CHANGE)DbgKdpPacket32);
        memcpy(&(DbgKdpPacket32[offsetof(DBGKD_WAIT_STATE_CHANGE, Context)]),
               &(DbgKdpPacket[offsetof(DBGKD_WAIT_STATE_CHANGE64, Context)]),
               PacketHeader.ByteCount - offsetof(DBGKD_WAIT_STATE_CHANGE64, Context));
        //printf("2 ByteCount delta %x\n", offsetof(DBGKD_WAIT_STATE_CHANGE64, Context)-offsetof(DBGKD_WAIT_STATE_CHANGE, Context));
        //KdDump(DbgKdpPacket32, PacketHeader.ByteCount);
        PacketHeader.ByteCount -= offsetof(DBGKD_WAIT_STATE_CHANGE64, Context)-offsetof(DBGKD_WAIT_STATE_CHANGE, Context);
        PacketHeader.PacketType = PACKET_TYPE_KD_STATE_CHANGE;
        //printf("3 ByteCount %x\n", PacketHeader.ByteCount);
        //KdDump(DbgKdpPacket32, PacketHeader.ByteCount);
        memcpy(DbgKdpPacket, DbgKdpPacket32, sizeof(DbgKdpPacket));
    } else
    if(DbgKdpUse64bit && PacketType == PACKET_TYPE_KD_STATE_MANIPULATE) {
        DbgkdManipulateState64To32((PDBGKD_MANIPULATE_STATE64)DbgKdpPacket, (PDBGKD_MANIPULATE_STATE)DbgKdpPacket32);
        memcpy(DbgKdpPacket, DbgKdpPacket32, sizeof(DbgKdpPacket));
        PacketHeader.ByteCount -= offsetof(DBGKD_WAIT_STATE_CHANGE64, Context)-offsetof(DBGKD_WAIT_STATE_CHANGE, Context);
    }
